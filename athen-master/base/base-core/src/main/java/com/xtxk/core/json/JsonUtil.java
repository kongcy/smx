...
import com.xtxk.core.util.Money;
...
public class JsonUtil {
    ...
    private static class RenderObjectMapper extends BasicObjectMapper {
        private RenderObjectMapper() {
            ...
            // 序列化枚举到前端时, 返回 value(没有就使用 name)和 code(没有就使用 ordinal), 前者用来显示,
            // 后者用来前后端传递
            ...
            // 序列化 PageList 时只将总条数和当前页的数据返回. 否则可以使用 PageListJsonSerializer
            // 这个现成的实现
     ...
     * 此方法只是为了在输出 json 字符串, 在 controller 中应该调用 {@link #toObjectWithField}
     * <p/>
     * <pre>
     * public class User {
     * 	Long id;
     ...
     *
     * // 输出
     * // {"id":123,"name":"ruby","password":"encrypt-code","info":{"id":890,"name":"abc123"}}
     ...
     * toJsonWithField(user, "*");
     *
     * // 输出 {"id":123,"name":"ruby","info":{}} <span style=
     * "color:red;">星号代表全部, 感叹(!) 和 减号(-) 都能排除属性</span>
