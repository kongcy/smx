...
public final class U {
    ...
    /**
     * 比较的equals，不会出现空指针异常的情况,区分大小写
     */
    public static boolean compareEquals(String source, String target){
        if (isBlank(source) && isBlank(target)) {
            return true;
        }
        if (isBlank(source) || isBlank(target)) {
    ...
    public static boolean compareNoEquals(String source, String target){
        return !compareEquals(source,target);
    }
    ...
    public static String delSuffix(String url) {
        if (U.isBlank(url)) {
            return EMPTY;
        }
        url = url.replaceAll("//","/");
